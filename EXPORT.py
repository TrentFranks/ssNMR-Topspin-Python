"""
Computes a TopSpin EXPORT wave shape:
...
def EXPORT_dialog():
   MAS=GETPAR("CNST 31")
   #MSG(MAS)
   input = INPUT_DIALOG("EXPORTinput", "", \
...
def EXPORTname(input):
   ...
   [Cname,Cwave,Nname,Nwave],["","","",""],["1","1","1","1"],["Accept","Close"], [ret,spc], 30))
   if Wave==None:EXIT
   ...
   PUTPAR(str(Cwave),str(Cfile))
   PUTPAR(str(Nwave),str(Nfile))
   
   return Cfile, Nfile

def EXPORTmake(input):
  import math
  ...
  for i in range(steps):
    ...
    if RFImax > (NomRFI*1000):
      ERRMSG(message="ERROR the maximum RF is above the nominal RF",\
      title="WAVE ERROR")
  ...
  Names=EXPORTname(input)
  SAVE_SHAPE(Names[0], "Excitation", Iampl, Iphases)
  SAVE_SHAPE(Names[1], "Excitation", Sampl, Sphases)
 
Export=EXPORT_dialog()
#Names=EXPORTname(Export)
wave=EXPORTmake(Export)
