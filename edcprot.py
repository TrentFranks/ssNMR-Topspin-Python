...
Find all active external protection files,
...
def get_all_prot(Text):
    ...
    while i < len(Text):
       ...
       if Text[i].find("include") >= 0 :
          ...
          l = Text[i].find(".")
          #print Text[i][k-4:k]
          if Text[i].find("prot") >= 0 :
             about = Text[i][j+1:k]
             files.append(about)
       i=i+1
    return files

def get_all_active_prot(Text):
    files = []
    i=0
    n=0
    while i < len(Text):
       ...
       if Text[i].find("include") >= 0 : 
          if Text[i].find(";") >= 0:                   #Commented in some way
	     if Text[i].find(";") > Text[i].find(">") :#Commented after (active)
                j = Text[i].find("<")
                k = Text[i].find(">")
		#print Text[i][l:k]
                if Text[i].find("prot") >= 0 :
...
fowl   = get_all_prot(gourds)
pig    = get_all_active_prot(gourds)
